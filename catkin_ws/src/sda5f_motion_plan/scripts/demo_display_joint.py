...
from sda5f_motion_plan.msg import OnRobotRGInput
from sda5f_motion_plan.msg import OnRobotVGInput
...
def save_rg6_status(status):
    """Saves the current rg6 status."""
    global rg6_status
    rg6_status = status.gSTA
    # rospy.loginfo("rg6: "+str(rg6_in_motion))
...
def demo_display():
    ...
    rospy.Subscriber("OnRobotRGInput", OnRobotRGInput, save_rg6_status)
    ...
    if sda5f_motion_possible==0:
        rospy.loginfo("Error on sda5f: Motion is not possible. Motor is not enabled.\n")
        rospy.signal_shutdown("Finished.")
        rospy.sleep(rospy.Duration.from_sec(1))
        return -1
    if (rg6_status==72) or (rg6_status==104):  # left (72) or right (104) finger's safety swith is activated
        rospy.loginfo("Error on rg6: Safety circuit 1 is activated so the gripper cannot move. Any of the safety switch is pushed.\n")
        rospy.signal_shutdown("Finished.")
        rospy.sleep(rospy.Duration.from_sec(1))
        return -1

    rospy.wait_for_service('/onrobot_rg/set_command')
    rospy.wait_for_service('/onrobot_vg/set_command')
    onrobot_rg_setcommand = rospy.ServiceProxy('/onrobot_rg/set_command', SetCommand)
    onrobot_vg_setcommand = rospy.ServiceProxy('/onrobot_vg/set_command', SetCommand)
    ...
    onrobot_rg_setcommand("700")  # reset
    onrobot_vg_setcommand('r')  # reset
    ...
    def leef_motions(is_ready_larm, is_ready_rarm, is_ready_leef, is_ready_reef, is_ready_torso):
        ...
        onrobot_rg_setcommand('c')
        wait_for_rg6_ready()
        is_ready_larm.put(True)
        wait_for_ready(is_ready_leef)
        rospy.loginfo("leef: open")
        onrobot_rg_setcommand("700")
        wait_for_rg6_ready()
        is_ready_larm.put(True)
    ...
    def reef_motions(is_ready_larm, is_ready_rarm, is_ready_leef, is_ready_reef, is_ready_torso):
        ...
        onrobot_vg_setcommand('g')
        is_ready_rarm.put(True)
        wait_for_ready(is_ready_reef)
        rospy.loginfo("reef: release")
        onrobot_vg_setcommand('r')
        is_ready_rarm.put(True)
    ...
    larm_thread = ThreadWithReturnValue(
        target=larm_motions, args=(
            is_ready_larm, is_ready_rarm, is_ready_leef, is_ready_reef, is_ready_torso))
    larm_thread.start()
    rarm_thread = ThreadWithReturnValue(
        target=rarm_motions, args=(
            is_ready_larm, is_ready_rarm, is_ready_leef, is_ready_reef, is_ready_torso))
    rarm_thread.start()
    leef_thread = ThreadWithReturnValue(
        target=leef_motions, args=(
            is_ready_larm, is_ready_rarm, is_ready_leef, is_ready_reef, is_ready_torso))
    leef_thread.start()
    reef_thread = ThreadWithReturnValue(
        target=reef_motions, args=(
            is_ready_larm, is_ready_rarm, is_ready_leef, is_ready_reef, is_ready_torso))
    reef_thread.start()
    torso_thread = ThreadWithReturnValue(
        target=torso_motions, args=(
            is_ready_larm, is_ready_rarm, is_ready_leef, is_ready_reef, is_ready_torso))
    torso_thread.start()
